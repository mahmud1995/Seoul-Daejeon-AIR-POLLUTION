...
for i in list(anti_rate_d):
    hisdata.append(i)
print(hisdata)
ozonedata=list(ozone_s)
for i in list(ozone_d):
    ozonedata.append(i)
print(ozonedata)

X = np.array(hisdata)
y = np.array(ozonedata)
w = 0.0
b = 0.0
...
ozone_pre = pd.read_csv("ozone predict data.csv", index_col="name")
...
# Function to calculate the estimated use of Antihistamin
def calculate_antihistamin():
    my_city = city_entry.get()
    
    # Check if the city exists in the data
    if my_city in ozone_pre.index and my_city in pop.index:
        a = ozone_pre.loc[my_city]
